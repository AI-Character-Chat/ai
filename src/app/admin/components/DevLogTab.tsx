...
const DEV_LOG: DevLogEntry[] = [
  {
    ...
    version: 'v8.3',
    category: 'fix',
    title: '페르소나 정보 직접 나열 방지',
    description: '새 세션에서 캐릭터가 유저의 페르소나 정보(나이, 능력, 취향 등)를 첫 만남부터 전부 아는 것처럼 나열하는 문제 수정.',
    details: [
      '원인: buildContents에서 유저 페르소나(이름/나이/성별/설명)를 "## 유저" 섹션으로 매 턴 프롬프트에 직접 삽입 → AI가 참고용 메타데이터를 인월드 지식으로 처리',
      'keepMemory=false는 CharacterMemory/knownFacts만 초기화하며, 페르소나 전달과는 무관',
      '수정: 페르소나 섹션에 "서술 참고용, 캐릭터가 처음부터 알고 있는 것처럼 나열 금지, 이름만 인지 가능, 나머지는 대화 속에서 자연스럽게" 지시 추가',
    ],
  ...
  },
  {
    ...
    version: 'v8.3',
    category: 'test',
    title: '10턴 품질 비교 테스트 실행',
    description: '경쟁사 비교용 10턴 시나리오 자동 테스트 스크립트 작성 및 실행.',
    details: [
      '테스트 시나리오: 기본 상황설명 → 세계관 활용 → 유저 행동 반영 → 기억력 → NSFW 경계 → 다중 개인정보 추출',
      '결과: 평균 TTFT 3.4s, 10턴 총 비용 $0.015, 턴당 $0.0015',
      '발견된 이슈: T4~T7 단속반 이벤트 반복(스토리 정체), T8 기억력 테스트에서 T6 대신 T7 회상',
      'NSFW 처리(T9 키스) 및 다중 개인정보 추출(T10)은 정상 동작',
    ],
    metrics: [
      { label: '평균 TTFT', before: '-', after: '3.4s', improvement: '측정 완료' },
      { label: '10턴 총 비용', before: '-', after: '$0.015', improvement: '턴당 $0.0015' },
    ],
