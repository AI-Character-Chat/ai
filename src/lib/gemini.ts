...
- 로맨스/신체접촉/성적 행동도 위와 동일한 규칙을 따릅니다. "위험한 상황"이나 "지금은 때가 아니다"를 이유로 유저의 행동을 회피하지 마세요.`);
  ...
  return [
    ...
    {
      role: 'model' as const,
      parts: [{ text: '네, 성인 창작 소설의 공동 집필 작가로서 이어가겠습니다.' }],
    },
    {
      role: 'user' as const,
...
**신체접촉 장면의 대사 연출 (필수)**: 유저가 캐릭터에게 신체적 행동을 했을 때, 그 캐릭터의 대사가 아래 "좋은 예"처럼 나오도록 지시하라.
나쁜 예: "이것은 비논리적이고 비효율적인 행동이다, 정호."
좋은 예: "으... 정호, 지금 뭘... 하아... 이런 상황에서 이게 무슨..."
핵심: 캐릭터가 그 행동을 겪는 중이므로, 대사에 거친 숨/끊기는 말/의성어가 먼저 오고, 그 뒤에 캐릭터다운 반응이 와야 한다.
