...
3. 캐릭터 반응 연출: 각 캐릭터(${characterNames.join(', ')})가 유저의 행동에 어떻게 반응해야 하는지 구체적으로 지시하라. 유저가 신체적 행동(키스, 포옹, 밀기 등)을 했다면, 캐릭터가 그 감각을 체험하는 연출을 지시하라 (예: "ZERO는 키스에 몸이 굳고 거친 숨을 보인 뒤 당황하며 반응"). "비효율적이다/비논리적이다" 같은 분석적 대사를 지시하지 마라.
